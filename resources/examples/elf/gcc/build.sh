...
build_arch() {
    ...
    "$GCC" -std=c23 -c -O0 -fno-pic -fno-pie "${CFLAGS_UNWIND[@]}" "${CFLAGS_ARCH[@]}" -o "$out_dir/minimal.o" "$SCRIPT_DIR/minimal.c"
    "$GCC" -std=c23 -c -O0 -fno-pic -fno-pie "${CFLAGS_UNWIND[@]}" "${CFLAGS_ARCH[@]}" -o "$out_dir/function.o" "$SCRIPT_DIR/function.c"
    "$GCC" -std=c23 -c -O0 -fno-pic -fno-pie "${CFLAGS_UNWIND[@]}" "${CFLAGS_ARCH[@]}" -o "$out_dir/data.o" "$SCRIPT_DIR/data.c"
    "$GCC" -std=c23 -c -O0 -fno-pic -fno-pie "${CFLAGS_UNWIND[@]}" "${CFLAGS_ARCH[@]}" -o "$out_dir/symbol-export.o" "$SCRIPT_DIR/symbol-export.c"
    "$GCC" -std=c23 -c -O0 -fno-pic -fno-pie "${CFLAGS_UNWIND[@]}" "${CFLAGS_ARCH[@]}" -o "$out_dir/symbol-import.o" "$SCRIPT_DIR/symbol-import.c"
    "$GCC" -std=c23 -c -O0 -fno-pic -fno-pie "${CFLAGS_UNWIND[@]}" "${CFLAGS_ARCH[@]}" -o "$out_dir/override-weak.o" "$SCRIPT_DIR/override-weak.c"
    "$GCC" -std=c23 -c -O0 -fno-pic -fno-pie "${CFLAGS_UNWIND[@]}" "${CFLAGS_ARCH[@]}" -o "$out_dir/override-strong.o" "$SCRIPT_DIR/override-strong.c"
    "$GCC" -std=c23 -c -O0 -fno-pic -fno-pie "${CFLAGS_UNWIND[@]}" "${CFLAGS_ARCH[@]}" -o "$out_dir/relocate-within-data.o" "$SCRIPT_DIR/relocate-within-data.c"
    ...
    "$GCC" -std=c23 -c -O0 -fno-pic -fPIE -ftls-model=local-exec "${CFLAGS_UNWIND[@]}" "${CFLAGS_ARCH[@]}" -o "$out_dir/tls.o" "$SCRIPT_DIR/tls.c"
    "$GCC" "${LDFLAGS_ARCH[@]}" -ftls-model=local-exec -o "$out_dir/tls.elf" "$out_dir/tls.o"

    "$GCC" -std=c23 -c -O0 -ftls-model=global-dynamic "${CFLAGS_UNWIND[@]}" "${CFLAGS_ARCH[@]}" -o "$out_dir/tls-gd.o" "$SCRIPT_DIR/tls.c"
    "$GCC" "${LDFLAGS_ARCH[@]}" -ftls-model=global-dynamic -o "$out_dir/tls-gd.elf" "$out_dir/tls-gd.o"

    "$GCC" -std=c23 -c -O0 -fno-pic -fPIE "${CFLAGS_UNWIND[@]}" "${CFLAGS_ARCH[@]}" -o "$out_dir/relocate-within-tls.o" "$SCRIPT_DIR/relocate-within-tls.c"
    "$GCC" "${LDFLAGS_ARCH[@]}" -o "$out_dir/relocate-within-tls.elf" "$out_dir/relocate-within-tls.o"

    "$GCC" -std=c23 -c -O0 -fPIC "${CFLAGS_UNWIND[@]}" "${CFLAGS_ARCH[@]}" -o "$out_dir/share-export.o" "$SCRIPT_DIR/share-export.c"
    "$GCC" -std=c23 -c -O0 -fPIC "${CFLAGS_UNWIND[@]}" "${CFLAGS_ARCH[@]}" -o "$out_dir/share-import.o" "$SCRIPT_DIR/share-import.c"
